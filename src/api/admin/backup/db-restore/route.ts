...
export async function POST(req: MedusaRequest<{ url?: string | undefined }>, res: MedusaResponse) {
  ...
  try {
    ...
    try {
      ...
      logger.error("⚠️ Could not check/create safe database, falling back to postgres database", checkError)
    ...
    try {
      ...
      try {
        ...
        logger.error("⚠️ Could not cleanup temp database:", cleanupError)
      }
    ...
    try {
      ...
      logger.error("⚠️ Failed to backup db_backups table (table might not exist)")
      logger.error(`Backup error: ${backupError.message}`)
    ...
    try {
      ...
      logger.warn("⚠️ Connection termination completed with warnings (this is normal)")
    ...
    if (fs.existsSync(backupRecordsPath)) {
      ...
      try {
        ...
        logger.warn("⚠️ Could not filter backup records, proceeding with original file")
    ...
    try {
      ...
      logger.warn("⚠️ Could not drop db_backups table (might not exist)")
    ...
    try {
      ...
      logger.warn("⚠️ Could not restore db_backups table")
      ...
      if (restoreTableError.message.includes("unrecognized configuration parameter")) {
        ...
        try {
          ...
          logger.warn("⚠️ Final db_backups restore attempt failed")
    ...
    logger.warn("⚠️ Keeping backup database for safety - you can manually drop it later if everything is working fine")
    logger.info(`💡 To remove backup database later, run: DROP DATABASE "${backupDbName}";`)
      ...
      try {
        ...
        if (tempDbName) {
          ...
          try {
            ...
            logger.error("⚠️ Could not cleanup temporary database:", cleanupError.message)
          }
        }
        ...
        try {
          ...
          logger.warn("⚠️ Connection termination during rollback completed with warnings")
        }

        await new Promise((resolve) => setTimeout(resolve, 1000))

        // Drop current (potentially corrupted) database
        logger.info("🗑️ Dropping corrupted database...")
        cmd = `psql "${MAINTENANCE_DB}" -c "DROP DATABASE IF EXISTS \\"${DB_NAME}\\";"`
        await execAsync(cmd)
        logger.info("✅ Corrupted database dropped")

        // Create fresh database
        logger.info("🏗️ Creating fresh database for rollback...")
